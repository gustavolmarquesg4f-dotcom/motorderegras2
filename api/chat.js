...
export default async function handler(req,res){
  ...
  const url=process.env.SUPABASE_URL,key=process.env.SUPABASE_PUBLISHABLE_KEY,aiKey=process.env.OPENAI_API_KEY,model=process.env.OPENAI_MODEL;
  if(!url||!key)return fail(res,503,'Banco ainda não configurado.');if(!aiKey||!model)return fail(res,503,'IA aguardando a chave e o modelo configurados no servidor.');
  ...
  const analysis=evaluate(record.payload);const snapshot={case:analysis.case,finance:analysis.finance,consignado:analysis.consignado,allocation:analysis.allocations,warnings:analysis.warnings,requestedStart:analysis.requestedStart,legal:analysis.legal};
